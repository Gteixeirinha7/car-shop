...
@WebServlet("/instalador/api/refresh-login")
public class APIRefreshLogin extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            if(!inputData.containsKey("clientId") || !inputData.containsKey("refreshToken")){
                throw new AppException(400, "Parâmetros Inválidos");
            }
            String clientId = inputData.get("clientId").toString().toLowerCase();
            String refreshToken = inputData.get("refreshToken").toString();
            response.getWriter().append(
                security.refreshLogin(clientId, refreshToken).toString()
            );
