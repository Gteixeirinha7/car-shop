...
@WebServlet("/instalador/api/reset-password")
public class APIResetPassword extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            if(!inputData.containsKey("id") || !inputData.containsKey("token") || !inputData.containsKey("password")){
                throw new AppException(400, "Parâmetros Inválidos");
            }
            String actionId = inputData.get("id").toString();
            String actionToken = inputData.get("token").toString();
            String password = inputData.get("password").toString();
            response.getWriter().append(
                security.resetPassword(actionId, actionToken, password).toString()
            );
