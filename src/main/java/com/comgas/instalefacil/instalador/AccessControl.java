...
public class AccessControl {
    ...
    public JSONObject signIn(String email, String password) throws AppException {
        ...
        try{
            ...
            if(rs.next()){
                throw new AppException(401, "Email já está Cadastrado");
            }
            this.userId = this.conn.getNextId("salesforce.instalefacil_instalador__c_id_seq");
            this.userSFId = "";
            status = "Inativo";
            ativacao = "0A";
            String encodedPassword = AppUtils.md5(email+password);
            this.conn.executeSQL(
                "INSERT INTO salesforce.instalefacil_instalador__c" +
                " (id, externalid__c, email__c, senha__c,"+
                " status_ativacao__c, status__c,"+
                " receber_pedidos__c, origem_aplicativo__c, isdeleted)" +
                " VALUES ("+ this.userId +
                        ","+this.conn.getUUIDClause(this.userId)+
                        ",'"+ this.conn.escape(email) +
                        "','"+ encodedPassword+
                        "','"+ativacao+
                        "','"+status+
                        "',true,true,false)"
    ...
    private void createLogin() throws AppException {
        this.clientId = this.conn.getNextId("salesforce.instalefacil_instalador_login_id_seq");
        this.refreshToken = AppUtils.md5(this.userSFId+this.clientId+this.userId+AppUtils.time());
        Long lastUseStamp = AppUtils.time();
        this.conn.executeSQL(
            "INSERT INTO salesforce.instalefacil_instalador_login" +
        ...
        try{
            ...
            if(rs.next()){
                if(this.refreshToken.equals(rs.getString("token"))){
                    ...
                    ativacao = rs.getString("status_ativacao__c");
                    this.conn.executeSQL(
                        "UPDATE salesforce.instalefacil_instalador_login " +
    ...
    private void createSession() throws AppException {
        this.sessionId = this.conn.getNextId("salesforce.instalefacil_instalador_session_id_seq");
        this.accessToken = AppUtils.md5(this.userSFId+this.sessionId+this.userId+AppUtils.time());
        Long lastUseStamp = AppUtils.time();
        this.conn.executeSQL(
            "INSERT INTO salesforce.instalefacil_instalador_session" +
            " (id, token, userid, lastusestamp, createddate, status)" +
    ...
    public void refreshSession(String sessionId, String accessToken) throws AppException {
        ...
        try{
            ResultSet rs = this.conn.executeQuery(
                ...
                " FROM salesforce.instalefacil_instalador_session A" +
                ", salesforce.instalefacil_instalador__c B"+
            ...
            if(rs.next()){
                if(this.accessToken.equals(rs.getString("token"))){
                    ...
                    this.isActive = true;
                    this.conn.executeSQL(
                        "UPDATE salesforce.instalefacil_instalador_session " +
                        " SET lastusestamp = " + lastUseStamp +
        ...
        if(this.accessToken.length() == 32 && this.sessionId > 0L){
            try{
                ResultSet rs = this.conn.executeQuery(
                    ...
                    " FROM salesforce.instalefacil_instalador_session " +
                    " WHERE id = " + this.sessionId
                );
                if(rs.next()){
                    if(this.accessToken.equals(rs.getString("token"))){
    ...
    public JSONObject sendEmailCheck() throws AppException, SQLException {
        if (this.isActive){
            this.createAction(
                //"EmailChecked", "/instalador/verificar-email.html",
                "EmailChecked", "/#/instalador/login/registro/verificar-email",
                this.userSFId, null, null
            );
        }else{
            throw new AppException(591, "Sessão Inválida");
        }
    ...
    public JSONObject emailChecked(String actionId, String actionToken) throws AppException{
        ...
        try{
            ResultSet rs = this.conn.executeQuery(
                "SELECT A.instalador__c, A.timestamp__c, B.id AS userid,"+
                      " B.sfid, B.status_ativacao__c, B.status__c"+
            ...
            if(rs.next()){
                ...
                ativacao = rs.getString("status_ativacao__c");
                status = rs.getString("status__c");
                if (ativacao.equals("0A")){
                    ativacao = "1A";
                    this.conn.executeSQL(
                         "UPDATE salesforce.instalefacil_instalador__c " +
                         " SET status_ativacao__c = '1A'"+
                         " WHERE id = " + this.userId +
                         " AND status_ativacao__c = '0A'"
                    );
                }
            ...
            throw new AppException(e.getMessage(), "AccessControl.emailChecked");
        }
        this.createLogin();
        this.createSession();
        JSONObject returnData = new JSONObject();
    ...
    public JSONObject sendPassword(String email) throws AppException {
        ...
        try{
            ...
            if(rs.next()){
                ...
                this.userSFId = rs.getString("sfid");
                this.createAction(
                    //"ResetPassword", "/instalador/criar-senha.html",
                    "ResetPassword", "/#/instalador/login/nova-senha",
                    this.userSFId, null, null
                );
            }else{
                throw new AppException(404, "E-mail não cadastrado!");
            }
        }catch (SQLException e){
    ...
    public JSONObject resetPassword(String actionId, String actionToken, String password)
        ...
        try{
            ...
            if(rs.next()){
                ...
                status = rs.getString("status__c");
                ativacao = rs.getString("status_ativacao__c");
                if (ativacao.equals("0A")) ativacao = "1A";
                String encodedPassword = AppUtils.md5(email+password);
                this.conn.executeSQL(
                     "UPDATE salesforce.instalefacil_instalador__c " +
                     " SET senha__c = '" + this.conn.escape(encodedPassword) + "'"+
                        ", status_ativacao__c = '" + this.conn.escape(ativacao) + "'"+
                     " WHERE id = " + this.userId
                );
                this.removeAction(actionId);
            }else{
