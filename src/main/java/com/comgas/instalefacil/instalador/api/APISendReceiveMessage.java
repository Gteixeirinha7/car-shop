...
@WebServlet("/instalador/api/send-receive-message")
    ...
    public JSONObject execute(JSONObject inputData) throws AppException, SQLException {
        ...
        if (inputData.containsKey("messages")){
            ...
            PreparedStatement insertMessages = this.getInsertStatement(
                ...
                "orcamento_id,message,sender='I',ispending=true,createddate=now()"
            );
            int n = 0;
        ...
        while(rs.next()){
            ...
            if (rs.getBoolean("ispending") && rs.getString("sender").equals("C")){
